...
public:
    ...
    void updateProcess(ST_FullSensorPayload_t& p_sensor_data) {
        ...
        int8_t 	v_result 		= _imu.getFIFOData(_fifoBuffer, &v_samplesRead);

        if (v_result != BMI2_OK || v_samplesRead == 0) return;
        ...
        for (uint16_t i = 0; i < v_samplesRead; i++) {
            ...
            if (_opts.useVQF && _vqf) {
                ...
                if (i == v_samplesRead - 1) {
                    p_sensor_data.rawAcc[0] 	= v_ax;
                    p_sensor_data.rawAcc[1] 	= v_ay;
                    p_sensor_data.rawAcc[2] 	= v_az;

                    p_sensor_data.gyro[0] 		= v_gx;
                    p_sensor_data.gyro[1] 		= v_gy;
                    p_sensor_data.gyro[2] 		= v_gz;
                    ...
                    p_sensor_data.linAcc[0] = v_ax - v_gx;
                    p_sensor_data.linAcc[1] = v_ay - v_gy;
                    p_sensor_data.linAcc[2] = v_az - v_gz;
        ...
        if (_opts.useSD && _sd && v_samplesRead > 0){
            char v_line[128];
            // 용량 최적화 포맷
            ...
            snprintf(v_line, sizeof(v_line), "%u,  %.3f,%.3f,%.3f,  %.4f,%.4f,%.4f,%.4f,   %.1f,%.1f,%.1f, %u,%d\n",
                     ...
                     p_sensor_data.euler[0] , p_sensor_data.euler[1] , p_sensor_data.euler[2],
...
private:
    ...
    void computeEuler(ST_FullSensorPayload_t& p_sensor_data) {
        ...
        p_sensor_data.euler[0] = atan2(2.0f * (v_quat_0 * v_quat_1 + v_quat_2 * v_quat_3), 1.0f - 2.0f * (v_quat_1 * v_quat_1 + v_quat_2 * v_quat_2)) * G_SB10_RAD_TO_DEG;
        p_sensor_data.euler[1] = asin(2.0f  * (v_quat_0 * v_quat_2 - v_quat_3 * v_quat_1)) * G_SB10_RAD_TO_DEG;
        p_sensor_data.euler[2] = atan2(2.0f * (v_quat_0 * v_quat_3 + v_quat_1 * v_quat_2), 1.0f - 2.0f * (v_quat_3 * v_quat_3 + v_quat_2 * v_quat_2)) * G_SB10_RAD_TO_DEG;
    }
    ...
    void resumeFromIdle() {
        ...
        if (_opts.useSD) {
            _sd->begin();
            _sd->createLogFile(_opts.logPrefix, "Time,Ax,Ay,Az,Gx,Gy,Gz,QW,QX,QY,QZ,Roll,Pitch,Yaw,StepCnt,Motion");
        }
    }
