...
bool T20_dspCacheEnsureTwiddle(CL_T20_Mfcc::ST_Impl* p, uint16_t p_fft_size)
{
    if (p == nullptr) {
        return false;
    }

    for (uint16_t i = 0; i < G_T20_DSP_TWIDDLE_CACHE_MAX_ITEMS; ++i) {
        if (p->twiddle_cache[i].valid && p->twiddle_cache[i].fft_size == p_fft_size) {
            return true;
        }
    }

    esp_err_t res = dsps_fft2r_init_fc32(NULL, p_fft_size);
    if (res != ESP_OK) {
        return false;
    }
    ...
    uint16_t slot = 0;
    for (uint16_t i = 0; i < G_T20_DSP_TWIDDLE_CACHE_MAX_ITEMS; ++i) {
        if (!p->twiddle_cache[i].valid) {
            slot = i;
            break;
        }
    }

    p->twiddle_cache[slot].valid = true;
    p->twiddle_cache[slot].fft_size = p_fft_size;
    return true;
}

bool T20_dspCacheFindMelBank(CL_T20_Mfcc::ST_Impl* p,
                             uint16_t p_profile_index,
                             uint16_t p_fft_size,
                             uint16_t p_mel_filters,
                             float p_sample_rate_hz,
                             float (**p_out_mel_bank)[(G_T20_FFT_SIZE / 2) + 1])
{
    if (p == nullptr || p_out_mel_bank == nullptr) {
        return false;
    }

    for (uint16_t i = 0; i < G_T20_DSP_CACHE_MAX_ITEMS; ++i) {
        ST_T20_DspCacheItem_t& v_item = p->dsp_cache[i];
        if (!v_item.valid) {
            continue;
        }
        if (v_item.profile_index == p_profile_index &&
            v_item.fft_size == p_fft_size &&
            v_item.mel_filters == p_mel_filters &&
            fabsf(v_item.sample_rate_hz - p_sample_rate_hz) < 0.001f) {
            *p_out_mel_bank = v_item.mel_bank;
            return true;
        }
    }
...
void T20_dspCacheStoreMelBank(CL_T20_Mfcc::ST_Impl* p,
                              uint16_t p_profile_index,
                              uint16_t p_fft_size,
                              uint16_t p_mel_filters,
                              float p_sample_rate_hz,
                              const float p_mel_bank[G_T20_MEL_FILTERS][(G_T20_FFT_SIZE / 2) + 1])
{
    if (p == nullptr) {
        return;
    }
    ...
    uint16_t v_slot = 0;
    for (uint16_t i = 0; i < G_T20_DSP_CACHE_MAX_ITEMS; ++i) {
        if (!p->dsp_cache[i].valid) {
            v_slot = i;
            break;
        }
    }

    ST_T20_DspCacheItem_t& v_item = p->dsp_cache[v_slot];
    v_item.valid = true;
    v_item.profile_index = p_profile_index;
    v_item.fft_size = p_fft_size;
    v_item.mel_filters = p_mel_filters;
    v_item.sample_rate_hz = p_sample_rate_hz;
    memcpy(v_item.mel_bank, p_mel_bank, sizeof(v_item.mel_bank));
}
...
void T20_buildMelFilterbank(CL_T20_Mfcc::ST_Impl* p, const ST_T20_Config_t* p_cfg)
{
    ...
    T20_dspCacheStoreMelBank(p,
                             p->current_profile_index,
                             p->cfg.feature.fft_size,
