...
bool T20_buildViewerMultiFrameJsonText(CL_T20_Mfcc::ST_Impl* p, char* p_out_buf, uint16_t p_len) { if (p==nullptr) return false; JsonDocument doc; doc["mode"]="viewer_multi_frame"; doc["count"]=p->viewer_recent_waveform_count; return T20_jsonWriteDoc(doc,p_out_buf,p_len); }
bool T20_buildViewerChartBundleJsonText(CL_T20_Mfcc::ST_Impl* p, char* p_out_buf, uint16_t p_len, uint16_t p_points) { (void)p; JsonDocument doc; doc["mode"]="viewer_chart_bundle"; doc["points"]=p_points; return T20_jsonWriteDoc(doc,p_out_buf,p_len); }
