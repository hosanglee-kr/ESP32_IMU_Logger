...
/* ============================================================================
[다음 단계 실제 연결 Hook Placeholder]
============================================================================ */
bool T20_bmi270ApplyRealBurstRead(CL_T20_Mfcc::ST_Impl* p)
{
    if (p == nullptr) return false;
    /* TODO: 실제 BMI270 burst read 연결 */
    return true;
}

bool T20_bmi270ApplyRealIsrAttach(CL_T20_Mfcc::ST_Impl* p)
{
    if (p == nullptr) return false;
    /* TODO: 실제 DRDY attachInterrupt 연결 */
    return true;
}

bool T20_recorderApplyRealWriteFinalize(CL_T20_Mfcc::ST_Impl* p)
{
    if (p == nullptr) return false;
    /* TODO: 실제 flush/close/finalize 연결 */
    return true;
}
/* ============================================================================
[향후 단계 구현 예정 정리 - Core]
1. BMI270 실제 SPI begin/init 연결
2. 실제 gyro/acc raw sample read 연결
3. DRDY ISR/queue 기반 실샘플 수집
4. hop_size 반영 frame build 마감
5. preview parser 컬럼 타입 추론 강화
6. unified viewer bundle 상세화
============================================================================ */

#include "T20_Mfcc_Inter_197.h"
#include <algorithm>
#include <vector>
...
bool T20_beginLiveSource(CL_T20_Mfcc::ST_Impl* p)
{
    ...
    if (!T20_configBMI270_1970Hz_DRDY(p)) return false;
    return true;
}
...
bool T20_configBMI270_1970Hz_DRDY(CL_T20_Mfcc::ST_Impl* p)
{
    if (p == nullptr) return false;
...
bool T20_buildRecorderFileBundleJsonText(CL_T20_Mfcc::ST_Impl* p, char* p_out_buf, uint16_t p_len)
{
    if (p == nullptr || p_out_buf == nullptr || p_len == 0) return false;
    JsonDocument doc;
    doc["store_bundle_state"] = p->recorder_store_bundle_state;
    doc["store_result"] = p->recorder_store_result;
    doc["file_write_state"] = p->recorder_file_write_state;
    doc["bundle_map_state"] = p->recorder_bundle_map_state;
    doc["manifest_state"] = p->recorder_manifest_state;
    doc["summary_state"] = p->recorder_summary_state;
    doc["index_state"] = p->recorder_index_state;
    doc["artifact_state"] = p->recorder_artifact_state;
...
bool T20_buildRecorderWriteFinalizeJsonText(CL_T20_Mfcc::ST_Impl* p, char* p_out_buf, uint16_t p_len)
{
    ...
    doc["file_write_state"] = p->recorder_file_write_state;
    doc["bundle_map_state"] = p->recorder_bundle_map_state;
    doc["path_route_state"] = p->recorder_path_route_state;
    doc["write_commit_state"] = p->recorder_write_commit_state;
    doc["write_finalize_state"] = p->recorder_write_finalize_state;
    doc["store_bundle_state"] = p->recorder_store_bundle_state;
    doc["store_result"] = p->recorder_store_result;
    return T20_jsonWriteDoc(doc, p_out_buf, p_len);
}
...
bool T20_buildRecorderCommitFinalizeJsonText(CL_T20_Mfcc::ST_Impl* p, char* p_out_buf, uint16_t p_len)
{
    ...
    doc["write_commit_state"] = p->recorder_write_commit_state;
    doc["write_finalize_state"] = p->recorder_write_finalize_state;
    doc["commit_route_state"] = p->recorder_commit_route_state;
    doc["finalize_sync_state"] = p->recorder_finalize_sync_state;
    return T20_jsonWriteDoc(doc, p_out_buf, p_len);
}
...
bool T20_buildHwAndFinalizeSyncJsonText(CL_T20_Mfcc::ST_Impl* p, char* p_out_buf, uint16_t p_len)
{
    if (p == nullptr || p_out_buf == nullptr || p_len == 0) return false;
    JsonDocument doc;
    doc["real_begin_state"] = p->bmi270_real_begin_state;
    doc["real_burst_state"] = p->bmi270_real_burst_state;
    doc["real_isr_state"] = p->bmi270_real_isr_state;
    doc["hw_link_state"] = p->bmi270_hw_link_state;
    doc["frame_build_state"] = p->bmi270_frame_build_state;
    doc["real_flush_state"] = p->recorder_real_flush_state;
    doc["real_close_state"] = p->recorder_real_close_state;
    doc["real_finalize_state"] = p->recorder_real_finalize_state;
    doc["meta_sync_state"] = p->recorder_meta_sync_state;
    doc["report_sync_state"] = p->recorder_report_sync_state;
    return T20_jsonWriteDoc(doc, p_out_buf, p_len);
}
