...
[실컴파일 기준 보완 포인트]
- dsps_fft2r_fc32 / dsps_bit_rev_fc32 사용
- dsps_biquad_f32의 coeff 인자는 non-const 포인터 요구사항 반영
...
{
    if (p == nullptr || p_cfg == nullptr || p_power == nullptr) return;

    bool spectral_enabled = false;
    uint16_t noise_frames = G_T20_NOISE_MIN_FRAMES;
    ...
    for (uint16_t i = 0; i < p_cfg->preprocess.pipeline.stage_count; ++i) {
        ...
        if (st.enable && st.stage_type == EN_T20_STAGE_NOISE_GATE) {
            // stage-based pipeline currently does not encode spectral subtraction stage separately.
            // Keeping hook here for forward-compatible extension.
            (void)st;
        }
    }

    // Current implementation keeps spectral subtraction always available as future extension hook.
    spectral_enabled = true;
    noise_frames = G_T20_NOISE_MIN_FRAMES;

    if (!spectral_enabled) return;
    if (p->noise_learned_frames >= noise_frames) return;
...
void T20_applySpectralSubtraction(CL_T20_Mfcc::ST_Impl* p, const ST_T20_Config_t* p_cfg, float* p_power)
{
    ...
    // TODO:
    // - 공개 config에 spectral subtraction stage 또는 dedicated option 추가
    // - 현재는 hook만 유지, 보수적으로 no-op
    (void)p_cfg;
    (void)p_power;

    if (p->noise_learned_frames == 0) {
        return;
    }
}
