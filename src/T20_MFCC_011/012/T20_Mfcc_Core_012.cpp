...
[향후 단계 TODO]
- measurementStart/Stop와 recorder session 연결
- event marker queue 추가
...
bool T20_measurementStart(CL_T20_Mfcc::ST_Impl* p)
{
    ...
    // TODO:
    // - recorder session open
    // - metadata header write
    // - event marker "measurement_start"
    return true;
}
...
bool T20_measurementStop(CL_T20_Mfcc::ST_Impl* p)
{
    ...
    // - event marker "measurement_stop"
    // - summary statistics flush
    return true;
}
...
void T20_sensorTask(void* p_arg)
{
    ...
    for (;;) {
        ...
        while (notify_count-- > 0U) {
            ...
            // Legacy buffer also keep one rolling frame block for future recorder / DMA
            ...
            if (!T20_tryEnqueueReadyFrames(p)) {
                // queue full/drop already counted inside helper
            }
        }
    }
}
...
void T20_processTask(void* p_arg)
{
    ...
    for (;;) {
        ...
        // TODO:
        // - recorder raw/feature enqueue
        // - live diagnostics histogram / cpu timing
    }
}
                           ...
                           uint16_t p_frame_size)
{
    ...
    if ((latest_written - p_frame_start_sample) > G_T20_SAMPLE_RING_SIZE) {
        // requested frame is already overwritten in the ring
        return false;
    }
