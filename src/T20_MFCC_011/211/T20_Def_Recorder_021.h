...
/* --- [Recorder Batch & Performance Constants] --- */
...
/* --- [Zero-Copy & DMA Buffer Constants] --- */
...
/* --- [Recorder State Transition Constants (All)] --- */
...
enum EM_T20_OutputMode_t {
    EN_T20_OUTPUT_VECTOR = 0, EN_T20_OUTPUT_SEQUENCE
};
...
typedef struct {
    ...
    uint32_t size_bytes; uint32_t created_ms; uint32_t record_count;
} ST_T20_RecorderIndexItem_t;

typedef struct {
    char     profile_name[G_T20_SDMMC_PROFILE_NAME_MAX];
    bool     use_1bit_mode; bool enabled;
    uint8_t  clk_pin; uint8_t cmd_pin; uint8_t d0_pin;
    uint8_t  d1_pin; uint8_t d2_pin; uint8_t d3_pin;
} ST_T20_SdmmcProfile_t;

typedef struct {
    uint32_t magic; uint16_t version; uint16_t header_size;
    uint32_t sample_rate_hz; uint16_t fft_size; uint16_t mfcc_dim;
    uint16_t mel_filters; uint16_t sequence_frames; uint32_t record_count;
    char     reserved[G_T20_BINARY_HEADER_RESERVED_BYTES];
} ST_T20_RecorderBinaryHeader_t;

typedef struct {
    uint32_t frame_id; uint16_t vector_len;
