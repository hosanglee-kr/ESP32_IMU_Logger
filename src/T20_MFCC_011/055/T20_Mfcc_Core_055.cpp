...
===============================================================================
[Web/Core 구조 정리 라운드]
- 이번 단계는 053 기준으로 남아 있을 가능성이 높은 범위/스코프/중괄호 혼선을 줄이는
  정리 라운드이다.
- Core는 helper 최소 구현과 선언 시그니처를 다시 맞추는 방향으로 정리한다.
- 실제 기능 고도화보다 "컴파일 가능한 단일 리비전 세트 정렬"을 우선한다.
===============================================================================
*/
...
{
    (void)p_path; (void)p_bytes;
    return T20_buildRecorderManifestJsonText(p, p_out_buf, p_len);
}
...
bool T20_buildRecorderRangeJsonText(CL_T20_Mfcc::ST_Impl* p, char* p_out_buf, uint16_t p_len, const char* p_path, uint32_t p_offset, uint32_t p_length)
{
    ...
    JsonDocument v_doc;
    v_doc["mode"] = "recorder_range";
    v_doc["path"] = p_path ? p_path : "";
    v_doc["offset"] = p_offset;
    v_doc["length"] = p_length;
    if (measureJson(v_doc) + 1 > p_len) return false;
    serializeJson(v_doc, p_out_buf, p_len);
    return true;
}
...
bool T20_buildViewerChartBundleJsonText(CL_T20_Mfcc::ST_Impl* p, char* p_out_buf, uint16_t p_len, uint16_t p_points)
{
    ...
    if (p_out_buf == nullptr || p_len == 0) return false;
    JsonDocument v_doc;
    v_doc["mode"] = "viewer_chart_bundle";
    v_doc["points"] = p_points;
    if (measureJson(v_doc) + 1 > p_len) return false;
    serializeJson(v_doc, p_out_buf, p_len);
    return true;
}
...
bool T20_buildRecorderBinaryHeaderJsonText(CL_T20_Mfcc::ST_Impl* p, char* p_out_buf, uint16_t p_len, const char* p_path)
{
    ...
    if (p_out_buf == nullptr || p_len == 0) return false;
    JsonDocument v_doc;
    v_doc["mode"] = "binary_header";
    v_doc["path"] = p_path ? p_path : "";
    v_doc["magic"] = G_T20_BINARY_MAGIC;
    v_doc["version"] = G_T20_BINARY_VERSION;
    if (measureJson(v_doc) + 1 > p_len) return false;
    serializeJson(v_doc, p_out_buf, p_len);
    return true;
}
...
bool T20_buildRecorderBinaryPayloadSchemaJsonText(CL_T20_Mfcc::ST_Impl* p, char* p_out_buf, uint16_t p_len, const char* p_path)
{
    ...
    JsonDocument v_doc;
    v_doc["mode"] = "binary_payload_schema";
    v_doc["vector_max"] = G_T20_FEATURE_DIM_MAX;
    if (measureJson(v_doc) + 1 > p_len) return false;
    serializeJson(v_doc, p_out_buf, p_len);
    return true;
}
