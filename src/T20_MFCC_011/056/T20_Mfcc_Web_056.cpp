#include "T20_Mfcc_Web_056.h"

/* ============================================================================
 * File: T20_Mfcc_Web_056.cpp
 * Summary:
 *   AsyncWeb 기반 T20 웹 API 등록
 *
 * [이번 버전 목적]
 * - Web 중괄호/스코프 붕괴를 피하는 단순하고 정적인 라우팅 구조
 * - 최소한의 helper와 API만 유지
 * ========================================================================== */
...
void T20_registerWebHandlers(CL_T20_Mfcc::ST_Impl* p, AsyncWebServer* v_server, const char* p_base_path)
{
    ...
    v_server->on(s_rec_csv_table_adv.c_str(), HTTP_GET, [p](AsyncWebServerRequest* request) {
        ...
        char json[G_T20_WEB_LARGE_JSON_BUF_SIZE] = {0};
        T20_sendJsonText(request,
                         T20_buildRecorderCsvTableAdvancedJsonText(p, json, sizeof(json), path, bytes, filter, col_filters_csv, sort_col, sort_dir, page, page_size),
                         json);
    });
