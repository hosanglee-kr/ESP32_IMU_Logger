#include "T20_Mfcc_Inter_043.h"

/*
===============================================================================
소스명: T20_Mfcc_Core_043.cpp
버전: v043
...
bool T20_applyConfigJsonText(CL_T20_Mfcc::ST_Impl* p, const char* p_json_text)
{
    ...
    ST_T20_Config_t v_cfg = p->cfg;
    String v_text = String(p_json_text);

    auto T20_findBool = [&](const char* p_key, bool p_old) -> bool {
        String v_pat = String("\"") + p_key + "\":";
        int v_pos = v_text.indexOf(v_pat);
        if (v_pos < 0) return p_old;
        String v_sub = v_text.substring(v_pos + v_pat.length(), min((int)v_text.length(), v_pos + (int)v_pat.length() + 8));
        v_sub.trim();
        return v_sub.startsWith("true") ? true : (v_sub.startsWith("false") ? false : p_old);
    };

    auto T20_findUint = [&](const char* p_key, uint16_t p_old) -> uint16_t {
        String v_pat = String("\"") + p_key + "\":";
        int v_pos = v_text.indexOf(v_pat);
        if (v_pos < 0) return p_old;
        int v_start = v_pos + v_pat.length();
        while (v_start < (int)v_text.length() && (v_text[v_start] == ' ' || v_text[v_start] == '\t')) v_start++;
        int v_end = v_start;
        while (v_end < (int)v_text.length() && isdigit((unsigned char)v_text[v_end])) v_end++;
        if (v_end <= v_start) return p_old;
        return (uint16_t)v_text.substring(v_start, v_end).toInt();
    };

    auto T20_findFloat = [&](const char* p_key, float p_old) -> float {
        String v_pat = String("\"") + p_key + "\":";
        int v_pos = v_text.indexOf(v_pat);
        if (v_pos < 0) return p_old;
        int v_start = v_pos + v_pat.length();
        while (v_start < (int)v_text.length() && (v_text[v_start] == ' ' || v_text[v_start] == '\t')) v_start++;
        int v_end = v_start;
        while (v_end < (int)v_text.length() && (isdigit((unsigned char)v_text[v_end]) || v_text[v_end] == '.' || v_text[v_end] == '-')) v_end++;
        if (v_end <= v_start) return p_old;
        return v_text.substring(v_start, v_end).toFloat();
    };

    v_cfg.feature.mfcc_coeffs = T20_findUint("mfcc_coeffs", v_cfg.feature.mfcc_coeffs);
    v_cfg.feature.delta_window = T20_findUint("delta_window", v_cfg.feature.delta_window);
    v_cfg.output.sequence_frames = T20_findUint("sequence_frames", v_cfg.output.sequence_frames);
    v_cfg.preprocess.preemphasis.enable = T20_findBool("preemphasis_enable", v_cfg.preprocess.preemphasis.enable);
    v_cfg.preprocess.preemphasis.alpha = T20_findFloat("preemphasis_alpha", v_cfg.preprocess.preemphasis.alpha);
    v_cfg.preprocess.filter.cutoff_hz_1 = T20_findFloat("cutoff_hz_1", v_cfg.preprocess.filter.cutoff_hz_1);
    v_cfg.preprocess.filter.cutoff_hz_2 = T20_findFloat("cutoff_hz_2", v_cfg.preprocess.filter.cutoff_hz_2);
    v_cfg.preprocess.filter.q_factor = T20_findFloat("q_factor", v_cfg.preprocess.filter.q_factor);
...
bool T20_buildRecorderManifestJsonText(CL_T20_Mfcc::ST_Impl* p, char* p_out_buf, uint16_t p_len)
{
    ...
    String v_json = "{";
    v_json += "\"index_path\":\"";
    v_json += G_T20_RECORDER_INDEX_PATH;
    v_json += "\",\"items\":[";
    bool v_first = true;
    for (uint16_t i = 0; i < p->recorder_index_count; ++i) {
        ST_T20_RecorderIndexItem_t* p_item = &p->recorder_index_items[i];
    ...
    v_json += "],\"todo\":\"TODO: 이후 단계에서 download manifest checksum/version/signature 확장\"}";
    if ((uint16_t)(v_json.length() + 1) > p_len) {
        return false;
    }
    memcpy(p_out_buf, v_json.c_str(), v_json.length() + 1);
...
bool T20_buildViewerDataJsonText(CL_T20_Mfcc::ST_Impl* p, char* p_out_buf, uint16_t p_len)
{
    ...
    String v_json = "{";
    v_json += "\"frame_id\":";
    v_json += String((unsigned long)p->viewer_last_frame_id);
    v_json += ",\"vector_len\":";
    v_json += String((unsigned)p->viewer_last_vector_len);
    v_json += ",\"vector\":[";
    for (uint16_t i = 0; i < p->viewer_last_vector_len; ++i) {
        if (i > 0) v_json += ",";
        v_json += String(p->viewer_last_vector[i], 6);
    }
    v_json += "]";
    v_json += ",\"todo\":\"TODO: 이후 단계에서 파형/스펙트럼/이벤트 시계열 API 확장\"}";
    if ((uint16_t)(v_json.length() + 1) > p_len) {
        return false;
    }
    memcpy(p_out_buf, v_json.c_str(), v_json.length() + 1);
...
bool T20_buildConfigJsonText(CL_T20_Mfcc::ST_Impl* p, char* p_out_buf, uint16_t p_len)
{
    ...
    JsonDocument v_doc;
    v_doc["version"] = G_T20_VERSION_STR;

    JsonObject v_pre = v_doc["preprocess"].to<JsonObject>();
    v_pre["axis"] = (int)p->cfg.preprocess.axis;
    v_pre["remove_dc"] = p->cfg.preprocess.remove_dc;

    JsonObject v_pe = v_pre["preemphasis"].to<JsonObject>();
    v_pe["enable"] = p->cfg.preprocess.preemphasis.enable;
    v_pe["alpha"] = p->cfg.preprocess.preemphasis.alpha;

    JsonObject v_filter = v_pre["filter"].to<JsonObject>();
    v_filter["enable"] = p->cfg.preprocess.filter.enable;
    v_filter["type"] = (int)p->cfg.preprocess.filter.type;
    v_filter["cutoff_hz_1"] = p->cfg.preprocess.filter.cutoff_hz_1;
    v_filter["cutoff_hz_2"] = p->cfg.preprocess.filter.cutoff_hz_2;
    v_filter["q_factor"] = p->cfg.preprocess.filter.q_factor;

    JsonObject v_noise = v_pre["noise"].to<JsonObject>();
    v_noise["enable_gate"] = p->cfg.preprocess.noise.enable_gate;
    v_noise["gate_threshold_abs"] = p->cfg.preprocess.noise.gate_threshold_abs;
    v_noise["enable_spectral_subtract"] = p->cfg.preprocess.noise.enable_spectral_subtract;
    v_noise["spectral_subtract_strength"] = p->cfg.preprocess.noise.spectral_subtract_strength;
    v_noise["noise_learn_frames"] = p->cfg.preprocess.noise.noise_learn_frames;

    JsonObject v_feat = v_doc["feature"].to<JsonObject>();
    v_feat["fft_size"] = p->cfg.feature.fft_size;
    v_feat["sample_rate_hz"] = p->cfg.feature.sample_rate_hz;
    v_feat["mel_filters"] = p->cfg.feature.mel_filters;
    v_feat["mfcc_coeffs"] = p->cfg.feature.mfcc_coeffs;
    v_feat["delta_window"] = p->cfg.feature.delta_window;

    JsonObject v_out = v_doc["output"].to<JsonObject>();
    v_out["output_mode"] = (int)p->cfg.output.output_mode;
    v_out["sequence_frames"] = p->cfg.output.sequence_frames;
    v_out["sequence_flatten"] = p->cfg.output.sequence_flatten;
