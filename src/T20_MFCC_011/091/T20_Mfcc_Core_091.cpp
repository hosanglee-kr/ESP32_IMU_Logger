...
bool T20_probeBMI270ChipId(CL_T20_Mfcc::ST_Impl* p)
{
    ...
    uint8_t chip_id = 0;
    if (!T20_bmi270ReadRegister(p, G_T20_BMI270_REG_CHIP_ID, &chip_id)) {
        p->bmi270_spi_ok = false;
        return false;
    }
...
bool T20_bmi270ReadVectorSample(CL_T20_Mfcc::ST_Impl* p, float* p_out_sample)
{
    ...
    if (!T20_bmi270ReadBurstSample(p, burst, sizeof(burst))) {
        return T20_readBMI270Sample(p, p_out_sample);
    }

    /* 한글 주석:
       현재 단계는 실제 raw decode 대신 burst read 성공 여부만 검증하고,
       축 모드에 따른 샘플은 임시 계산값으로 유지합니다. */
    return T20_readBMI270Sample(p, p_out_sample);
}
