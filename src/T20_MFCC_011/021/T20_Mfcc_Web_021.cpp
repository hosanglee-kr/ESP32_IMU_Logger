...
3. 설정 JSON 적용 보강(숫자/불리언/열거형 문자열)
4. periodic push hook 유지
5. live status/latest/wave/sequence 통합 payload 송신
6. 향후 단계 TODO 주석 보강
...
void T20_webPeriodicPush(CL_T20_Mfcc::ST_Impl* p)
{
    ...
    if ((v_now - p->web_last_push_ms) < G_T20_WEB_PUSH_INTERVAL_MS) {
        return;
    }
