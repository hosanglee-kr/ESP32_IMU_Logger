                                    ...
                                    uint16_t p_count)
{
    ...
    uint8_t v_slot = T20_selectRecorderWriteBufferSlot(p);
    (void)v_slot; /* 현재 단계는 zero-copy slot skeleton만 유지 */
    memset(p->recorder_zero_copy_stage, 0, sizeof(p->recorder_zero_copy_stage));
    /* TODO: 이후 단계에서 stage buffer에 batch binary를 먼저 구성 후 zero-copy write 적용 */
...
bool T20_tryMountSdmmcRecorderBackend(CL_T20_Mfcc::ST_Impl* p)
{
    ...
    /*
     * 보드별 1-bit mode 기본 마운트 시도
     * TODO:
     * - ESP32-S3 보드별 pin map/profile 분리
     * - 1-bit / 4-bit mode 선택 옵션화
     */
...
uint8_t T20_selectRecorderWriteBufferSlot(CL_T20_Mfcc::ST_Impl* p)
{
    ...
    uint8_t v_slot = p->recorder_zero_copy_slot_index;
    p->recorder_zero_copy_slot_index++;
    if (p->recorder_zero_copy_slot_index >= G_T20_RECORDER_ZERO_COPY_SLOT_MAX) {
        p->recorder_zero_copy_slot_index = 0;
    }
    return v_slot;
}
...
/*
TODO:
- zero-copy slot별 DMA/cache aligned staging buffer 실제 적용
- selection sync 상태와 preview/downsample cache 연동
- SD_MMC board hint를 웹 설정/JSON 설정과 연결
*/
...
===============================================================================
[컴파일/안정성 보완 메모]
- 이번 단계에서는 header 중복 선언 정리와 런타임 텍스트 처리 보완을 함께 반영했다.
- SD_MMC 경로는 '/sdcard/...' 형식 전제를 유지한다.
- 실제 보드별 pin/mode/profile 안정화는 다음 단계 TODO로 유지한다.
===============================================================================
*/

/*
TODO:
- SD_MMC board profile(JSON/웹 설정)과 mount 정책 연결
- zero-copy slot별 실제 DMA/cache aligned write buffer 적용
- batch 누적 flush와 sector/block 크기 정합 최적화
- render selection sync와 preview/downsample cache 실제 연계
*/
...
bool T20_writeRecorderBinaryHeader(File& p_file, const ST_T20_Config_t* p_cfg)
{
    ...
    v_hdr.magic            = G_T20_BINARY_MAGIC;
    v_hdr.version          = G_T20_BINARY_VERSION;
    v_hdr.header_size      = sizeof(ST_T20_RecorderBinaryHeader_t);
...
/*
TODO:
- sdmmc_profile을 JSON/웹 설정에서 선택 가능하게 확장
- board hint와 sdmmc_profile을 실제 mount 파라미터에 반영
- zero-copy stage buffer에 batch binary record를 미리 조립하는 경로 구현
- selection sync와 recorder preview cache를 공용 index로 연결
*/
