...
{
    p_out.println(F("------ T20 TypeMeta Status ------"));
    p_out.println(F("현재 단계: CSV 타입 메타 추론 + preview bridge skeleton"));
    p_out.println(F("향후 단계: 정렬/필터/다운샘플 preview 실제 연동"));
    p_out.println(F("---------------------------------"));
}
                                          ...
                                          uint16_t p_len)
{
    ...
    JsonDocument v_doc;
    v_doc["mode"] = "type_meta_preview_link";
    v_doc["preview_link_max"] = G_T20_TYPE_META_PREVIEW_LINK_MAX;
    v_doc["todo"] = "TODO: 이후 단계에서 타입 메타 캐시와 정렬/필터/다운샘플 preview 실제 연결";

    JsonObject v_sd = v_doc["sdmmc_profile"].to<JsonObject>();
    v_sd["profile_name"] = p->sdmmc_profile.profile_name;
    v_sd["enabled"] = p->sdmmc_profile.enabled;
    v_sd["use_1bit_mode"] = p->sdmmc_profile.use_1bit_mode;
