...
namespace T20 {
    // --- [1.1] 시스템 기본 제어 상수 ---
    namespace C10_Sys {
        inline constexpr char const* VERSION_STR         = "T20_Mfcc_v220";     // 시스템 펌웨어 식별 버전
    ...
    namespace C10_Path {
        ...
        inline constexpr char const* FILE_CFG_JSON    = "/sys/runtime_cfg_220_010.json"; // 런타임 환경 설정 파일 저장 경로
        ...
        inline constexpr char const* WEB_INDEX        = "index_220_010.html";            // 웹서버 기본 접속 화면 파일명
...
// --- [2.5] DSP 전처리(Pre-process) 하위 구조체 ---
typedef struct {
...
typedef struct {
    bool     auto_start;                // 부팅 시 즉시 수집 및 분석 시작 여부
...
// --- [2.7] 시스템 통합 관리 (Master Config) 구조체 ---
typedef struct {
...
static inline ST_T20_Config_t T20_makeDefaultConfig() {
    ...
    cfg.feature.hop_size                            = T20::C10_DSP::FFT_SIZE; // 100% 윈도우 점프 (No-Overlap)
