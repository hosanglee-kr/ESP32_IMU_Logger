...
function initCharts() {
    ...
    // v217: 파형 및 멜 스펙트럼은 최적화를 위해 제외되었을 수 있으므로 기본값 유지, MFCC만 주로 갱신됨
...
// v217: 엔드포인트 변경 (/calibrate_sensor -> /calibrate)
...
async function loadSettings() {
    try {
        ...
        // v217: Sequence Mode Toggle 추가 대응
...
async function saveSettings() {
    ...
    formData.forEach((value, key) => {
        ...
        } else if (key === 'wifi_use_static' || key === 'output_sequence') {
...
// v217: 생략된 /live_debug 및 /bmi270_bridge_state 대신 통합된 /status API를 활용하여 폴링
async function fetchDiagnostics() {
    try {
        ...
        // 미니 대시보드 갱신
        document.getElementById('val-fps').innerText = "N/A (v217)"; // v217 최적화로 제외
