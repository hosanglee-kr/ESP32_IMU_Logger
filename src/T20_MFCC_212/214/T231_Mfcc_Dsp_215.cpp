/* ============================================================================
 * File: T231_Mfcc_Dsp_215.cpp
 * Summary: MFCC 알고리즘 및 DSP 파이프라인 (ESP32-S3 SIMD 및 Mel 스케일 완벽 적용본)
...
bool T20_initDSP(CL_T20_Mfcc::ST_Impl* p) {
    ...
    // [보완] 진정한 비선형 Mel 스케일 필터뱅크 생성 (선형 버그 수정)
    float min_mel = T20_hzToMel(0.0f);
    float max_mel = T20_hzToMel(p->cfg.feature.sample_rate_hz / 2.0f);
    float mel_step = (max_mel - min_mel) / (G_T20_MEL_FILTERS + 1U);

    uint16_t bin_points[G_T20_MEL_FILTERS + 2];
    for(uint16_t i = 0; i < G_T20_MEL_FILTERS + 2; i++) {
        float hz = T20_melToHz(min_mel + (i * mel_step));
        bin_points[i] = (uint16_t)roundf(G_T20_FFT_SIZE * hz / p->cfg.feature.sample_rate_hz);
    }
...
void T20_dsps_wind_hamming_f32(float *data, int len) {
     // (len - 1)로 나누는 이유는 윈도우를 대칭(Symmetric)으로 만들기 위함입니다.
...
void T20_applyRuntimeFilter(CL_T20_Mfcc::ST_Impl* p, const float* p_in, float* p_out, uint16_t p_len) {
    ...
    // ESP-DSP SIMD 가속 Biquad 필터 적용 (내부적으로 s3_ae32 명령어 활용)
...
// [보완] 헤더의 원형 선언과 정확히 일치하도록 복구된 FFT 엔진
void T20_computePowerSpectrum(CL_T20_Mfcc::ST_Impl* p, const float* p_time, float* p_power) {
    ...
    alignas(16) static float fft_input[G_T20_FFT_SIZE * 2];
    ...
    for (int i = 0; i < bins; i++) {
        ...
        p_power[i] = (mag_sq < G_T20_EPSILON) ? G_T20_EPSILON : mag_sq;
    }
}
...
void T20_applySpectralSubtraction(CL_T20_Mfcc::ST_Impl* p, float* p_power) {
    ...
    for (uint16_t i = 0; i < bins; ++i) {
        ...
        if (sub_result < G_T20_EPSILON) sub_result = G_T20_EPSILON;
...
void T20_pushMfccHistory(CL_T20_Mfcc::ST_Impl* p, const float* p_mfcc, uint16_t p_dim) {
    ...
    if (p->mfcc_history_count < G_T20_MFCC_HISTORY) {
        ...
        for (int i = 1; i < G_T20_MFCC_HISTORY; i++) {
...
void T20_computeDeltaFromHistory(CL_T20_Mfcc::ST_Impl* p, uint16_t p_dim, uint16_t p_delta_window, float* p_delta_out) {
    if (!p || !p_delta_out || p->mfcc_history_count < G_T20_MFCC_HISTORY) return;
...
void T20_computeDeltaDeltaFromHistory(CL_T20_Mfcc::ST_Impl* p, uint16_t p_dim, float* p_delta2_out) {
    if (!p || !p_delta2_out || p->mfcc_history_count < 5) return; 
    const int t = 2; 
    for (uint16_t i = 0; i < p_dim; i++) {
...
void T20_computeMFCC(CL_T20_Mfcc::ST_Impl* p, const float* p_frame, float* p_mfcc_out) {
    ...
    dsps_mul_f32(p->work_frame, p->window, p->work_frame, G_T20_FFT_SIZE, 1, 1, 1);
