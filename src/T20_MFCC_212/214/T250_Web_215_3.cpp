/* ============================================================================
 * File: T250_Web_215_3.cpp
 * Summary: Web API 엔드포인트 및 핸들러 구현 (WebSocket 스트리밍 및 CORS 통합 완성본)
 * ========================================================================== */
...
static void T20_sendJsonText(AsyncWebServerRequest* request, bool ok, const char* json_ok) {
    ...
    if (ok && json_ok != nullptr) {
        response = request->beginResponse(200, T250_Web::MIME_JSON, json_ok);
    } else {
        response = request->beginResponse(500, T250_Web::MIME_JSON, T250_Web::JSON_FAIL);
    }
...
static void T20_sendJsonDocument(AsyncWebServerRequest* request, const JsonDocument& doc) {
    ...
    AsyncWebServerResponse* response = request->beginResponse(200, T250_Web::MIME_JSON, json);
    response->addHeader("Access-Control-Allow-Origin", "*");
    request->send(response);
}
...
void T20_registerFileStreamingHandler(CL_T20_Mfcc::ST_Impl* p, AsyncWebServer* v_server, const String& base) {
    v_server->on((base + "/recorder/download").c_str(), HTTP_GET, [p](AsyncWebServerRequest* request) {
        ...
        if (!T20_getQueryParamPath(request, "path", path, sizeof(path))) {
            AsyncWebServerResponse *response = request->beginResponse(400, T250_Web::MIME_TEXT, "path_required");
            response->addHeader("Access-Control-Allow-Origin", "*");
            request->send(response);
            return;
        }
        ...
        if (!file) {
            AsyncWebServerResponse *response = request->beginResponse(404, T250_Web::MIME_TEXT, "file_not_found");
            response->addHeader("Access-Control-Allow-Origin", "*");
            request->send(response);
            return;
        }
...
void T20_registerStaticFrontendHandlers(AsyncWebServer* v_server) {
    ...
    v_server->onNotFound([](AsyncWebServerRequest *request) {
        if (request->method() == HTTP_OPTIONS) {
            ...
            AsyncWebServerResponse *response = request->beginResponse(404, T250_Web::MIME_TEXT, "404: Not Found in LittleFS");
            response->addHeader("Access-Control-Allow-Origin", "*");
            request->send(response);
        }
    });
