...
function showTab(tabId) {
    ...
    // 버튼 클릭에 의한 이벤트인 경우에만 active 클래스 추가
...
function connectWebSocket() {
    ...
    socket.onmessage = (event) => {
        if (event.data instanceof ArrayBuffer) {
            ...
            charts.wave.data.datasets[0].data = floats.slice(0, 256);
            charts.spec.data.datasets[0].data = floats.slice(256, 385);
            charts.mfcc.data.datasets[0].data = floats.slice(385, 424);
...
async function saveSettings() {
    ...
    try {
        ...
        if(res.ok) showToast("Settings successfully applied!");
