 ...
 * ============================================================================
 * File: T450_FsmMgr_004.cpp
 * Summary: FSM Orchestrator & Multi-task Implementation
 * ========================================================================== */

#include "T450_FsmMgr_004.hpp"
#include "esp_log.h"
#include <cstring>
...
void T450_FsmManager::_captureTask(void* p_param) {
    ...
    const uint32_t v_hopSamples = (uint32_t)(SmeaConfig::System::SAMPLING_RATE_CONST * (SmeaConfig::Dsp::HOP_MS_DEF / 1000.0f)); 
...
void T450_FsmManager::_processingTask(void* p_param) {
    ...
    while(1) {
        ...
        if (xQueueReceive(v_this->_qReadySlotIdx, &v_slotIdx, pdMS_TO_TICKS(100))) {
            SmeaType::FeatureSlot& v_slot = v_this->_featurePool[v_slotIdx];
            SmeaType::RawDataSlot& v_raw  = v_this->_rawPool[v_slotIdx];
            ...
            if (v_this->_systemState == SystemState::RECORDING) {
                if (!v_this->_isManualRecording && (millis() - v_this->_recordStartMs > (SmeaConfig::Decision::VALID_END_SEC_DEF * 1000))) {
            ...
            v_this->_communicator.broadcastBinary(v_slot.mfcc, sizeof(v_slot.mfcc));
...
DetectionResult T450_FsmManager::_runHybridDecision(uint8_t p_slotIdx) {
    ...
    if (v_slot.energy < SmeaConfig::Decision::TEST_NG_MIN_ENERGY_DEF) return DetectionResult::TEST_NG;
    
    // Priority 1: Rule NG 검문
    if (v_slot.energy > SmeaConfig::Decision::RULE_ENRG_THRESHOLD_DEF) return DetectionResult::RULE_NG;
    
    if (v_slot.pooling_stddev_min > SmeaConfig::Decision::RULE_STDDEV_THRESHOLD_DEF) return DetectionResult::RULE_NG;
