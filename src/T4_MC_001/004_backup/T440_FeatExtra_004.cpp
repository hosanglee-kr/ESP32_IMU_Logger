...
#include "T440_FeatExtra_004.hpp"
#include "esp_log.h"
...
void T440_FeatureExtractor::_computePowerSpectrum(const float* p_signal, uint32_t p_len) {
    ...
    if (_isNoiseLearning || _noiseLearnedFrames < SmeaConfig::Dsp::NOISE_LEARN_FRAMES_DEF) {
    ...
    if (!_isNoiseLearning && _noiseLearnedFrames > 0) {
        for (uint16_t i = 0; i < v_bins; i++) {
            _powerSpectrum[i] = fmaxf(_powerSpectrum[i] - (_noiseProfile[i] * SmeaConfig::Dsp::SPECTRAL_SUB_GAIN_DEF), 1e-12f);
        }
    }
}
...
void T440_FeatureExtractor::_computeBandRMS(SmeaType::FeatureSlot& p_slot) {
    ...
    for (int b = 0; b < SmeaConfig::Feature::BAND_RMS_COUNT_DEF; b++) {
        float v_startHz = SmeaConfig::Feature::BAND_RANGES_DEF[b][0];
        float v_endHz = SmeaConfig::Feature::BAND_RANGES_DEF[b][1];
