...
#include "T450_FsmMgr_004.hpp" // FSM 상태 확인 및 명령 하달용
#include <LittleFS.h>
#include <SD_MMC.h>
#include <time.h> 
#include <Update.h> 
...
bool T470_Communicator::init(const char* p_ssid, const char* p_pw, const char* p_mqttBroker) {
    strncpy(_creds.ssid, p_ssid, sizeof(_creds.ssid));
    strncpy(_creds.password, p_pw, sizeof(_creds.password));
    strncpy(_creds.mqtt_broker, p_mqttBroker, sizeof(_creds.mqtt_broker));
    _creds.use_mqtt = (strlen(p_mqttBroker) > 0);
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        v_startMs = millis();
        while (!getLocalTime(&v_timeinfo, 100) && (millis() - v_startMs < SmeaConfig::Network::NTP_TIMEOUT_MS_DEF)) {
            delay(100);
        }
    } else {
    ...
    if (_creds.use_mqtt && isConnected()) {
        _mqttClient.setServer(_creds.mqtt_broker, SmeaConfig::Mqtt::DEFAULT_PORT_DEF);
    }
...
void T470_Communicator::_reconnectMqtt() {
    if (millis() - _lastMqttRetryMs > SmeaConfig::Mqtt::RETRY_INTERVAL_MS_DEF) {
