...
public:
    ...
    // 파일 시스템 초기화 및 JSON 로드 (없으면 기본값 생성)
    bool init();
    
    // JSON 파일에서 구조체로 역직렬화 (Deserialization)
    bool load();
    
    // 구조체 값을 JSON 파일로 원자적(Atomic) 직렬화 저장
    bool save();
...
private:
    ...
    // T410_Config_004.hpp의 _DEF 값들을 _config 구조체로 복사
    void _loadDefaults();
};
