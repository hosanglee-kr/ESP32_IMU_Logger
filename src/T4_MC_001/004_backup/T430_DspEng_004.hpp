...
public:
     ...
     * @param p_len 처리할 샘플 길이 (보통 SmeaConfig::System::FFT_SIZE_CONST)
     */
    void process(float* p_micL, float* p_micR, float* p_output, uint32_t p_len);

private:
    // [서브루틴] DSP 수학적 파이프라인 컴포넌트 (private 멤버 함수 _ 접두사 적용)
    void _applyMedianFilter(float* p_data, int p_windowSize, uint32_t p_len);
    void _removeDC(float* p_data, uint32_t p_len);
    void _applyNoiseGate(float* p_data, uint32_t p_len);
    void _applyPreEmphasis(float* p_data, uint32_t p_len);
    void _applyBeamforming(const float* p_L, const float* p_R, float* p_out, uint32_t p_len);
