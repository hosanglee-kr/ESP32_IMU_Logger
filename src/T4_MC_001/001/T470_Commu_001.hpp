 ...
 * - 원칙: v_isOtaRunning 플래그로 동시 다발적인 OTA 접근을 HTTP 423 Locked로 방어할 것.
...
private:
    AsyncWebServer v_server;
    AsyncWebSocket v_ws;
    WiFiClient     v_wifiClient;
    PubSubClient   v_mqttClient;

    uint32_t v_lastMqttRetryMs = 0;
    uint32_t v_lastWifiRetryMs = 0; // 논블로킹 WiFi 타이머
...
private:
    ...
    void setCorsHeaders(AsyncWebServerResponse* p_response);
    void sendJsonResponse(AsyncWebServerRequest* p_request, const JsonDocument& p_doc);
    void reconnectMqtt();
};
