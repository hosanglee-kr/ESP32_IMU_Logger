 ...
 * * [AI 메모: 마이그레이션 적용 사항]
 * - 버퍼 및 파일 경로 길이를 담당하던 128, 16 매직 넘버 철폐 완료.
 * - 회전 및 용량 계산에 쓰이는 수학적 단위 변환 매직 넘버 철폐 완료.
 * ========================================================================== */
#include "T460_Storage_011.hpp"
...
void T460_StorageManager::_allocatePreBuffer() {
    ...
    // 오버랩(Overlap) 환경에 맞는 정확한 1초당 프레임 수 도출 (상수 적용)
    uint32_t v_fps = (uint32_t)(SmeaConfig::System::MS_PER_SEC_CONST / v_cfg.dsp.hop_ms);
...
bool T460_StorageManager::_loadIndexJson() {
    ...
    for (JsonObject v_item : v_arr) {
        ...
        strlcpy(_indexItems[i].path, v_item["path"] | "", SmeaConfig::StorageLimit::MAX_PATH_LEN_CONST);
        strlcpy(_indexItems[i].raw_path, v_item["raw_path"] | "", SmeaConfig::StorageLimit::MAX_PATH_LEN_CONST);
...
void T460_StorageManager::_handleRotation() {
    ...
    while (_indexCount > SmeaConfig::StorageLimit::ROTATE_KEEP_MAX_CONST) {
        char v_oldPath[SmeaConfig::StorageLimit::MAX_PATH_LEN_CONST];
        char v_oldRawPath[SmeaConfig::StorageLimit::MAX_PATH_LEN_CONST];
...
void T460_StorageManager::checkRotation() {
    ...
    // [수정] MB 변환, 시간 변환 상수를 사용하여 매직 넘버 철폐
    if (_writtenBytes >= (v_cfg.storage.rotate_mb * SmeaConfig::StorageLimit::BYTES_PER_MB_CONST)) v_doRotate = true;
    if ((millis() - _sessionStartMs) >= (v_cfg.storage.rotate_min * SmeaConfig::StorageLimit::MS_PER_MIN_CONST)) v_doRotate = true;
