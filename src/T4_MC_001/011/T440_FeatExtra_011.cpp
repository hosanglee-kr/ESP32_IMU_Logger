/* ============================================================================
 * File: T440_FeatExtra_011.cpp
 * Summary: 39D MFCC, Cepstrum & Spatial Feature Extraction (Math-Corrected)
 * ============================================================================
 * * [AI 메모: 적용된 수학적 맹점 극복 및 최적화 목록 (21종 마스터플랜 기반)]
 * 1. [방어] 공간 위상 분석 시 L/R에 Hanning Window 적용하여 스펙트럼 누출 차단.
 * 2. [교정] O(N) 슬라이딩 윈도우 스캔 방식의 STA/LTA 탐색 (돌발 충격음 100% 포착).
 * 3. [교정] Parseval 정리: FFT 파워에 단방향(2.0) 및 윈도우 감쇠(2.666) 보정값 적용.
 * 4. [방어] SIMD NaN 독성 확산 방지: 패딩 버퍼 memset 0.0f 초기화.
 * 5. [교정] 부동소수점 흡수를 막기 위한 EMA 기반 능동 노이즈(ANC) 프로파일링.
 * 6. [교정] Spectral Centroid 도출 시 0Hz(DC) 바이어스 배제.
 * 7. [교정] N-Top Peak 도출 전 NMS 로직 및 Threshold 방어 적용.
 * 8. [교정] Cepstrum 단위 변환 에러 교정 및 복소수 에너지 정밀 연산.
 * 9. [최적화] Band RMS 도출 시 평균(Average)이 아닌 총합(Total Power)으로 변경.
 * 10. [방어] 첨도(Kurtosis) 연산 언더플로우 방어 및 초과 첨도(-3.0) 맵핑 적용.
 * ========================================================================== */

#include "T440_FeatExtra_011.hpp"
#include "esp_log.h"
...
void T440_FeatureExtractor::_computeBasicFeatures(const float* p_signal, uint32_t p_len, SmeaType::FeatureSlot& p_slot) {
    ...
    // [수학적 교정] O(N) 슬라이딩 윈도우 스캔 기반의 강력한 STA/LTA (임펄스 누락 원천 차단)
    ...
    // [수학적 교정] 첨도 언더플로우 방어 (1e-6 이상일 때만 수행) 및 초과 첨도(-3.0) 계산
...
void T440_FeatureExtractor::_computePowerSpectrum(const float* p_signal, uint32_t p_len) {
    ...
    // [수학적 교정] Parseval 정리 보정: 단방향 에너지 보존(2.0) 및 Hanning 윈도우 감쇠(2.666) 복원
    ...
    // [방어] SIMD 행렬 곱 연산 시 NaN 독성이 퍼지는 것을 막기 위해 패딩 공간 0.0f로 클리어
    ...
    // [수학적 교정] 부동소수점 흡수(Absorption)를 막는 지수이동평균(EMA) 능동 소음 프로파일링
    if (_isNoiseLearning) {
        float v_alpha = 0.01f; // 100프레임 (약 1초) 시정수
        for (uint16_t i = 0; i < v_bins; i++) {
...
void T440_FeatureExtractor::_computeSpectralCentroid(SmeaType::FeatureSlot& p_slot) {
    ...
    // [수학적 교정] DC 성분(i=0) 바이어스 배제: 1부터 루프를 돌아 소리의 순수한 특성만 산출
...
// ----------------------------------------------------------------------------
// [수학적 교정 적용] NMS 기반 피크 탈락 & 임계치 방어 적용
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
// [수학적 교정 적용] Quefrency 역산, DC 회피 및 복소수 절대 에너지 연산
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
// [수학적 교정 적용] 스펙트럼 누출 방어용 L/R Hanning Window 선행 적용
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
// [수학적 교정 적용] Bin 평균값(Average)이 아닌 밴드 총 에너지(Total Power)로 비교
// ----------------------------------------------------------------------------
void T440_FeatureExtractor::_computeBandRMS(SmeaType::FeatureSlot& p_slot) {
    ...
    for (uint8_t b = 0; b < v_cfg.feature.band_rms_count; b++) {
        ...
        p_slot.band_rms[b] = sqrtf(v_sumSq); // 밴드 폭 무관 총 에너지 산출
    }
}
