...
namespace SmeaConfig {
    ...
    namespace System {
        ...
        inline constexpr float      PCM_32BIT_SCALE_CONST    = 1.0f / 2147483648.0f;   // 32bit PCM -> Float 변환 스케일 상수 (1.0 / 2^31)
        inline constexpr float      MATH_EPSILON_CONST       = 1e-6f;                  // 부동소수점 0 나누기 연산 방어용 Epsilon
        inline constexpr float      MATH_EPSILON_12_CONST    = 1e-12f;                 // 0 나누기 방어 정밀도 
        inline constexpr float      MS_PER_SEC_CONST         = 1000.0f;                // 밀리초 단위를 초 단위로 변환하기 위한 상수
    ...
    namespace Hardware {
        ...
        inline constexpr int        I2S_DMA_BUF_COUNT_CONST  = 8;                      // I2S DMA 버퍼 개수 
    }
    ...
    namespace Task {
        ...
        inline constexpr uint32_t   WDG_YIELD_MS_CONST       = 1;
        inline constexpr uint32_t   REBOOT_DELAY_MS_CONST    = 500;
        
        
        inline constexpr uint32_t   BOOT_DELAY_MS_CONST      = 1000;	// 시스템 부팅 시 시리얼 포트 등 하드웨어 안정화 대기 시간
        inline constexpr uint32_t   MAIN_LOOP_DELAY_MS_CONST = 10;      // 메인 루프(Core 1)의 백그라운드 태스크 양보 지연 시간
    }
    
    namespace DspLimit {
        inline constexpr int        MAX_MEDIAN_WINDOW_CONST  = 15;      // Median 필터 최대 윈도우 크기 (스택 오버플로우 방어용 상한선)
    ...
    namespace StorageLimit {
        ...
        inline constexpr uint16_t   MAX_PATH_LEN_CONST       = 128;      // 파일 절대 경로 및 문자열 버퍼 최대 길이
        inline constexpr uint16_t   MAX_PREFIX_LEN_CONST     = 16;       // 세션 접두어(prefix) 문자열 버퍼 최대 길이
        inline constexpr uint32_t   BYTES_PER_MB_CONST       = 1048576;  // 메가바이트(MB) -> 바이트(Bytes) 변환 상수 (1024 * 1024)
        inline constexpr uint32_t   MS_PER_MIN_CONST         = 60000;    // 분(Minute) -> 밀리초(ms) 변환 상수
    }
    ...
    namespace FeatureLimit {
        ...
        inline constexpr uint16_t   DELTA_HISTORY_FRAMES_CONST = 5;          // N=2 Delta 연산 버퍼
        inline constexpr uint32_t   STA_SAMPLES_CONST          = 42;         // 단기 에너지 윈도우(1ms)
        inline constexpr uint32_t   LTA_SAMPLES_CONST          = 420;        // 장기 에너지 윈도우(10ms)
    ...
    namespace DecisionLimit {
        ...
        inline constexpr uint8_t    MAX_TRIAL_COUNT_CONST      = 3;          // 검증 시나리오 최대 반복 횟수 (워밍업 포함)
    }
    ...
    namespace NetworkLimit {
        ...
        inline constexpr uint8_t    MAX_MULTI_AP_CONST              = 3;   // 다중 접속 지원 AP 최대 개수
        inline constexpr uint16_t   MAX_BROKER_LEN_CONST            = 64;  // MQTT 브로커 주소 최대 길이
        inline constexpr uint32_t   WIFI_MODE_SWITCH_DELAY_MS_CONST = 50;  // WiFi 하드웨어 모드 전환 대기 시간 (ms)
        inline constexpr uint32_t   WIFI_DISCONNECT_DELAY_MS_CONST  = 100; // WiFi 기존 연결 해제 대기 시간 (ms)
    }
    ...
    namespace Feature {
        ...
        inline constexpr uint8_t   BAND_RMS_COUNT_DEF = 4; // 실제 런타임에 추출/순회할 활성 밴드 개수 (JSON으로 변경 가능)
        
        // 여유 공간(MAX 8)을 확보해둔 밴드 범위 배열 기본값
    ...
    namespace Network {
        ...
        // 공장 초기화용 WiFi 기본 설정값
        inline constexpr uint8_t WIFI_MODE_DEF = 3; // 3: AUTO_FALLBACK
