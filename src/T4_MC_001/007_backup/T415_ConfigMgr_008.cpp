/* ============================================================================
 * File: T415_ConfigMgr_008.cpp
 * Summary: Dynamic JSON Configuration Manager Implementation
 * * [AI 메모: 마이그레이션 적용 사항]
 * 1. 버퍼 크기 매직넘버(32, 64, 16) 전면 _CONST 상수화 적용.
 * 2. AP 기본 설정("SMEA_100_AP" 등) 전면 _DEF 상수화 적용.
 * 3. 이전 단계(T440, T450)에서 도출했던 ceps_targets, sta_lta_threshold 등 신규 필드 누락 방지.
 * ========================================================================== */
#include "T415_ConfigMgr_008.hpp"
#include "esp_log.h"
...
void T415_ConfigManager::_loadDefaults() {
    ...
    _config.decision.sta_lta_threshold = 3.0f; // 기본값
