 ...
 * -> (방어) 부동소수점 0 나누기 방어는 fabsf(val) < MATH_EPSILON_CONST 검사를 쓰고, 
 * NaN/Inf 유입은 IEEE-754 비트 마스킹으로 하드웨어 컷을 수행한다.
 * 3. [네이밍 컨벤션 엄수]: private(_), 매개변수(p_), 로컬변수(v_)
 * 4. [동적 설정 연동 방어]: 런타임에 결정되는 윈도우 크기가 정적 버퍼 크기를 
 * 초과하여 스택을 파괴(Overflow)하지 않도록 MAX_MEDIAN_WINDOW_CONST로 상한을 둔다.
 * ============================================================================
 * File: T430_DspEng_007.hpp
