/* ============================================================================
 * File: T430_DspEng_007.cpp
 * Summary: SIMD Optimized DSP Implementation (Dynamic Config Applied)
 * * [AI 메모: 마이그레이션 적용 사항]
 * - Median Filter 스택 오버플로우 원천 차단 (MAX_MEDIAN_WINDOW_CONST 도입).
 * - FIR Window 크기 및 Epsilon 매직 넘버 철폐 완료.
 * ========================================================================== */
...
void T430_DspEngine::_removeDC(float* p_data, uint32_t p_len) {
    ...
    for (uint32_t i = 0; i < p_len; i++) {
        ...
        // IEEE-754 마스킹 (NaN/Inf 비트 패턴). 수식 상수이므로 하드코딩 유지
...
void T430_DspEngine::_applyMedianFilter(float* p_data, int p_windowSize, uint32_t p_len) {
    ...
    // [방어] 동적 설정값이 정적 스택 버퍼 크기를 초과하지 않도록 안전 Clamp 처리
    ...
    // [매직넘버 철폐] 고정된 7, 3 배열 크기 대신 상한(MAX_MEDIAN) 상수로 정적 할당
...
void T430_DspEngine::_generateFirLpfWindowedSinc(float* p_coeffs, uint16_t p_taps, float p_cutoffHz) {
    ...
    // [매직넘버 철폐] 임의의 128 크기 대신 정확한 탭(Taps) 상수로 메모리 안전 확보
