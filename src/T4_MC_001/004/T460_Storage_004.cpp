...
void T460_StorageManager::_allocatePreBuffer() {
    ...
    uint32_t v_fps = (uint32_t)(1000.0f / SmeaConfig::Dsp::HOP_MS_DEF); 
    uint16_t v_reqCapacity = v_fps * SmeaConfig::Storage::PRE_TRIGGER_SEC_DEF; // 100 * 3 = 300 프레임 확보
...
void T460_StorageManager::checkIdleFlush() {
    xSemaphoreTakeRecursive(_lock, portMAX_DELAY);
    if (_sessionOpen && _batchCount > 0 && (millis() - _lastPushMs) >= SmeaConfig::Storage::IDLE_FLUSH_MS_DEF) {
        flush();
    }
    xSemaphoreGiveRecursive(_lock);
}
...
void T460_StorageManager::checkRotation() {
    ...
    if (_writtenBytes >= (SmeaConfig::Storage::ROTATE_MB_DEF * 1024 * 1024)) v_doRotate = true;
    if ((millis() - _sessionStartMs) >= (SmeaConfig::Storage::ROTATE_MIN_DEF * 60000)) v_doRotate = true;
