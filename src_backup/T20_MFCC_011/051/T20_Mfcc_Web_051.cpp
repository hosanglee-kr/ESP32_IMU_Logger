...
bool T20_webAttach(CL_T20_Mfcc::ST_Impl* p, void* p_server, const char* p_base_path)
{
    ...
    String s_cfg_schema = String(v_base) + "/config/schema";
    ...
    String s_rec_file = String(v_base) + "/recorder/file";
    String s_rec_summary = String(v_base) + "/recorder/summary";
    String s_rec_meta = String(v_base) + "/recorder/meta";
    String s_rec_page = String(v_base) + "/recorder/page";
    String s_rec_range = String(v_base) + "/recorder/file_range";
    String s_rec_tail = String(v_base) + "/recorder/tail";
    String s_viewer = String(v_base) + "/viewer/state";
    ...
    String s_viewer_multi = String(v_base) + "/viewer/multiframe";
    String s_rec_stream = String(v_base) + "/recorder/file_stream";
    String s_rec_preview = String(v_base) + "/recorder/file_preview";
    String s_rec_preview_parsed = String(v_base) + "/recorder/file_preview_parsed";
    String s_rec_range = String(v_base) + "/recorder/file_range_json";
    String s_rec_header = String(v_base) + "/recorder/file_binary_header";
    String s_rec_csv_table = String(v_base) + "/recorder/file_csv_table";
    String s_rec_csv_table_filtered = String(v_base) + "/recorder/file_csv_table_filtered";
    String s_rec_csv_table_column_filtered = String(v_base) + "/recorder/file_csv_table_column_filtered";
    ...
    String s_rec_binary_records = String(v_base) + "/recorder/file_binary_records";
    String s_rec_binary_payload_schema = String(v_base) + "/recorder/file_binary_payload_schema";

    String s_rec_session_open_binary = String(v_base) + "/recorder/session_open_binary";
    String s_rec_session_finalize_binary = String(v_base) + "/recorder/session_finalize_binary";
    String s_rec_request_flush = String(v_base) + "/recorder/request_flush";
    String s_rec_request_finalize = String(v_base) + "/recorder/request_finalize";

    String s_rec_partial = String(v_base) + "/recorder/file_partial";
    String s_viewer_chart = String(v_base) + "/viewer/chart_bundle";
    ...
    v_server->on(s_rec_file.c_str(), HTTP_GET, [p](AsyncWebServerRequest* request) {
        char v_path[G_T20_WEB_PATH_BUF_SIZE] = {0};
        if (!T20_getQueryParamPath(request, "path", v_path, sizeof(v_path))) {
            request->send(400, "application/json; charset=utf-8", "{\"ok\":false,\"reason\":\"missing path\"}");
            return;
        }
        if (!LittleFS.exists(v_path)) {
            request->send(404, "application/json; charset=utf-8", "{\"ok\":false,\"reason\":\"not found\"}");
            return;
        }
        request->send(LittleFS, v_path, "application/octet-stream", true);
    });

    v_server->on(s_rec_summary.c_str(), HTTP_GET, [p](AsyncWebServerRequest* request) {
        char v_path[G_T20_WEB_PATH_BUF_SIZE] = {0};
        if (!T20_getQueryParamPath(request, "path", v_path, sizeof(v_path))) {
            request->send(400, "application/json; charset=utf-8", "{\"ok\":false,\"reason\":\"missing path\"}");
            return;
        ...
        request->send(LittleFS, v_path, "application/json; charset=utf-8", false);
    });

    v_server->on(s_rec_meta.c_str(), HTTP_GET, [p](AsyncWebServerRequest* request) {
        char v_path[G_T20_WEB_PATH_BUF_SIZE] = {0};
        if (!T20_getQueryParamPath(request, "path", v_path, sizeof(v_path))) {
            request->send(400, "application/json; charset=utf-8", "{\"ok\":false,\"reason\":\"missing path\"}");
            return;
    ...
    v_server->on(s_viewer.c_str(), HTTP_GET, [p](AsyncWebServerRequest* request) {
        char v_json[512] = {0};
        snprintf(v_json, sizeof(v_json),
                 "{\"running\":%s,\"recorder_enabled\":%s,\"recorder_index_count\":%u,\"todo\":\"TODO: 이후 단계에서 파형/스펙트럼/sequence 시각화 데이터 API 확장\"}",
                 p->running ? "true" : "false",
                 p->recorder_enabled ? "true" : "false",
                 (unsigned)p->recorder_index_count);
    ...
    v_server->on(s_cfg_schema.c_str(), HTTP_GET, [p](AsyncWebServerRequest* request) {
        char v_json[768] = {0};
...
void T20_webDetach(CL_T20_Mfcc::ST_Impl* p)
{
    ...
    v_server->on(s_rec_session_open_binary.c_str(), HTTP_POST, [p](AsyncWebServerRequest* request) {
        char v_path[G_T20_WEB_PATH_BUF_SIZE] = {0};
        if (!T20_getBodyPathOrDefault(request, v_path, sizeof(v_path), "/t20_session.bin")) {
            strlcpy(v_path, "/t20_session.bin", sizeof(v_path));
        }

        bool v_ok = T20_tryOpenRecorderBinarySession(p, v_path);
        request->send(v_ok ? 200 : 500, "application/json; charset=utf-8",
                      v_ok ? "{\"ok\":true,\"message\":\"binary session opened\"}" :
                             "{\"ok\":false,\"message\":\"binary session open failed\"}");
    });


    v_server->on(s_rec_request_finalize.c_str(), HTTP_POST, [p](AsyncWebServerRequest* request) {
        p->recorder_finalize_requested = true;
        request->send(200, "application/json; charset=utf-8", "{\"ok\":true,\"message\":\"finalize requested\"}");
    });
