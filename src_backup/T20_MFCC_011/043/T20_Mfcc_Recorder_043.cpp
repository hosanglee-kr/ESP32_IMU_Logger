...
bool T20_recorderMaybeRotate(CL_T20_Mfcc::ST_Impl* p)
{
    ...
    // 이번 단계:
    // - raw / meta / event / feature를 같은 rotate index로 함께 회전
    // - config 파일은 세션 기본 설정 파일로 유지
    // 향후 단계 TODO:
    // - rotate 목록 관리 파일 추가
    // - 시간 기반 rotate 정책 추가
    // - 오래된 rotate 파일 정리 정책 추가

    char v_rot_raw[192] = {0};
    char v_rot_meta[192] = {0};
    char v_rot_event[192] = {0};
    char v_rot_feature[192] = {0};

    uint32_t v_idx = (uint32_t)(millis() / 1000UL);

    snprintf(v_rot_raw, sizeof(v_rot_raw), "%s.%lu.rot", p->recorder.raw_path, (unsigned long)v_idx);
    snprintf(v_rot_meta, sizeof(v_rot_meta), "%s.%lu.rot", p->recorder.meta_path, (unsigned long)v_idx);
    snprintf(v_rot_event, sizeof(v_rot_event), "%s.%lu.rot", p->recorder.event_path, (unsigned long)v_idx);
    snprintf(v_rot_feature, sizeof(v_rot_feature), "%s.%lu.rot", p->recorder.feature_path, (unsigned long)v_idx);
    ...
    if (SD_MMC.exists(p->recorder.raw_path))     { SD_MMC.remove(v_rot_raw);     SD_MMC.rename(p->recorder.raw_path, v_rot_raw); }
    if (SD_MMC.exists(p->recorder.meta_path))    { SD_MMC.remove(v_rot_meta);    SD_MMC.rename(p->recorder.meta_path, v_rot_meta); }
    if (SD_MMC.exists(p->recorder.event_path))   { SD_MMC.remove(v_rot_event);   SD_MMC.rename(p->recorder.event_path, v_rot_event); }
    if (SD_MMC.exists(p->recorder.feature_path)) { SD_MMC.remove(v_rot_feature); SD_MMC.rename(p->recorder.feature_path, v_rot_feature); }

    p->recorder.raw_file = SD_MMC.open(p->recorder.raw_path, FILE_WRITE);
    p->recorder.meta_file = SD_MMC.open(p->recorder.meta_path, FILE_WRITE);
    p->recorder.event_file = SD_MMC.open(p->recorder.event_path, FILE_WRITE);
    p->recorder.feature_file = SD_MMC.open(p->recorder.feature_path, FILE_WRITE);
...
===============================================================================
[향후 단계 TODO]
- recorder queue 적체 시 기록 우선순위 정책 세분화
- binary block batching + CRC 추가
- session 종료 시 summary JSON 자동 생성
===============================================================================
*/
...
===============================================================================
[추가 구현]
- recorder binary header 정식 writer 골격
- 현재는 향후 recorder open 단계에서 호출할 수 있도록 helper만 제공
- 실제 open/start 경로와의 강결합은 다음 단계에서 연결 예정
===============================================================================
*/
bool T20_writeRecorderBinaryHeader(File& p_file, const ST_T20_Config_t* p_cfg)
{
    if (!p_file || p_cfg == nullptr) {
        return false;
    }
    ...
    v_hdr.sample_rate_hz = (uint32_t)p_cfg->feature.sample_rate_hz;
    v_hdr.fft_size = p_cfg->feature.fft_size;
    v_hdr.mfcc_dim = p_cfg->feature.mfcc_coeffs;
    v_hdr.mel_filters = p_cfg->feature.mel_filters;
    v_hdr.sequence_frames = p_cfg->output.sequence_frames;
    v_hdr.record_count = 0;

    size_t v_written = p_file.write((const uint8_t*)&v_hdr, sizeof(v_hdr));
    return (v_written == sizeof(v_hdr));
}
                                  ...
                                  uint32_t p_timestamp_ms)
{
    ...
    if (p->recorder_state != EN_T20_RECORDER_OPEN &&
    ...
    File v_file = LittleFS.open(p->recorder_file_path, "a");
    if (!v_file) {
        return false;
    }

    bool v_ok = T20_writeRecorderBinaryVectorRecord(v_file, p_timestamp_ms, p_vector, p_vector_len);
    v_file.close();

    if (v_ok) {
        p->recorder_state = EN_T20_RECORDER_RECORDING;
        p->recorder_record_count++;
        p->recorder_last_timestamp_ms = p_timestamp_ms;
