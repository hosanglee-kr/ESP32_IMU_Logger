...
function getCsvColumnFiltersCsv(){
  return getCsvColumnFilters().join('|');
}

async function loadRecorderCsvTableColumnFiltered(){
  try{
    const path = document.getElementById('rec-preview-path')?.value || '';
    const filter = String(document.getElementById('csv-filter')?.value || '');
    const sortCol = Number(document.getElementById('csv-sort-col')?.value || 0);
    const pageSize = Number(document.getElementById('csv-page-size')?.value || 10);
    const colFilters = getCsvColumnFiltersCsv();

    const url = '/api/t20/recorder/file_csv_table_column_filtered?path=' + encodeURIComponent(path)
      + '&filter=' + encodeURIComponent(filter)
      + '&col_filters=' + encodeURIComponent(colFilters)
      + '&sort_col=' + encodeURIComponent(sortCol)
      + '&page=0'
      + '&page_size=' + encodeURIComponent(pageSize);
    ...
    const el = document.getElementById('rec-csv-table-json');
    if(el){ el.textContent = 'CSV 서버 컬럼필터 조회 실패: ' + e; }
  }
}

async function requestRecorderFinalizePreview(){
  try{
    const r = await fetch('/api/t20/recorder/request_finalize', { method:'POST' });
    const t = await r.text();
    const el = document.getElementById('rec-binary-payload-schema-json');
    if(el){ el.textContent = t; }
  }catch(e){
    const el = document.getElementById('rec-binary-payload-schema-json');
    if(el){ el.textContent = 'Recorder finalize 요청 실패: ' + e; }
  }
}

window.addEventListener('DOMContentLoaded', ()=>{
  const bcf = document.getElementById('btn-load-rec-csv-table-column-filtered');
  if(bcf){ bcf.addEventListener('click', loadRecorderCsvTableColumnFiltered); }

  const brfin = document.getElementById('btn-rec-request-finalize');
  if(brfin){ brfin.addEventListener('click', requestRecorderFinalizePreview); }
});


async function loadRecorderCsvTableAdvanced(){
  try{
    const path = document.getElementById('rec-preview-path')?.value || '';
    const filter = String(document.getElementById('csv-filter')?.value || '');
    const sortCol = Number(document.getElementById('csv-sort-col')?.value || 0);
    const sortDir = Number(document.getElementById('csv-sort-dir')?.value || 0);
    const pageSize = Number(document.getElementById('csv-page-size')?.value || 10);
    const colFilters = getCsvColumnFiltersCsv();

    const url = '/api/t20/recorder/file_csv_table_advanced?path=' + encodeURIComponent(path)
      + '&filter=' + encodeURIComponent(filter)
      + '&col_filters=' + encodeURIComponent(colFilters)
      + '&sort_col=' + encodeURIComponent(sortCol)
    ...
    const el = document.getElementById('rec-csv-table-json');
    if(el){ el.textContent = 'CSV 고급 조회 실패: ' + e; }
  }
}


function syncChartViewAcrossCanvases(){
  const zoom = Number(document.getElementById('chart-zoom')?.value || 1);
  const pan = Number(document.getElementById('chart-pan')?.value || 0);
  const info = document.getElementById('chart-selection-info');
  if(info){
    const base = info.textContent || '';
    info.textContent = base.split('|')[0].trim() + ' | sync zoom=' + zoom + ' pan=' + pan + ' | render-sync ready';
  }
}

window.addEventListener('DOMContentLoaded', ()=>{
  const badv = document.getElementById('btn-load-rec-csv-table-advanced');
  if(badv){ badv.addEventListener('click', loadRecorderCsvTableAdvanced); }

  const sortDir = document.getElementById('csv-sort-dir');
  if(sortDir){ sortDir.addEventListener('change', renderCsvTableInteractive); }
});


window.__t20ChartSyncState = {
  zoom: 1,
  pan: 0
};

function updateSharedChartSyncState(){
  window.__t20ChartSyncState.zoom = Number(document.getElementById('chart-zoom')?.value || 1);
  window.__t20ChartSyncState.pan = Number(document.getElementById('chart-pan')?.value || 0);
}

function applySharedChartSyncState(){
  const z = document.getElementById('chart-zoom');
  const p = document.getElementById('chart-pan');
  if(z){ z.value = String(window.__t20ChartSyncState.zoom); }
  if(p){ p.value = String(window.__t20ChartSyncState.pan); }
  applyChartView();
}

async function loadRecorderCsvTypeMeta(){
  try{
    const path = document.getElementById('rec-preview-path')?.value || '';
    const url = '/api/t20/recorder/file_csv_type_meta?path=' + encodeURIComponent(path);
    const r = await fetch(url);
    const j = await r.json();
    const el = document.getElementById('rec-csv-table-json');
    if(el){ el.textContent = JSON.stringify(j, null, 2); }
    ...
    const el = document.getElementById('rec-csv-table-json');
    if(el){ el.textContent = 'CSV 타입 메타 조회 실패: ' + e; }
  }
}

window.addEventListener('DOMContentLoaded', ()=>{
  const btm = document.getElementById('btn-load-rec-csv-type-meta');
  if(btm){ btm.addEventListener('click', loadRecorderCsvTypeMeta); }

  const z = document.getElementById('chart-zoom');
  const p = document.getElementById('chart-pan');
  if(z){ z.addEventListener('input', ()=>{ updateSharedChartSyncState(); syncChartViewAcrossCanvases(); }); }
  if(p){ p.addEventListener('input', ()=>{ updateSharedChartSyncState(); syncChartViewAcrossCanvases(); }); }
});
