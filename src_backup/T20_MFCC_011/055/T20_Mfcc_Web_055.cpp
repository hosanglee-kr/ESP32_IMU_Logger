...
void T20_registerWebHandlers(CL_T20_Mfcc::ST_Impl* p, AsyncWebServer* v_server, const char* p_base_path)
{
    ...
    v_server->on(s_rec_range.c_str(), HTTP_GET, [p](AsyncWebServerRequest* request) {
        ...
        if (request->hasHeader("Range")) {
            const AsyncWebHeader* v_header = request->getHeader("Range");
            if (v_header != nullptr) {
        ...
        AsyncWebServerResponse* v_resp = request->beginResponse(200, "application/octet-stream", (const uint8_t*)v_buf.get(), v_read);
        if (v_resp != nullptr) {
