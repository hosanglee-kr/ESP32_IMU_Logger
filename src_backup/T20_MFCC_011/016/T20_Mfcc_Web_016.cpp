...
/*
===============================================================================
소스명: T20_Mfcc_Web_016.cpp
버전: v016

[기능 스펙]
- AsyncWeb 기반 상태 조회 API
- 측정 시작/종료 API
- 최신 특징 조회 API
- 세션 파일 다운로드 API

[현재 단계 설계]
- 공개 헤더의 의존성 최소화를 위해 내부에서만 AsyncWebServer 사용
- route handler는 현재 인스턴스의 내부 상태를 직접 읽어 JSON 응답 생성
- 설정 변경 POST body 파싱은 다음 단계 TODO로 남김
===============================================================================
*/
...
bool T20_webAttach(CL_T20_Mfcc::ST_Impl* p, void* p_server, const char* p_base_path)
{
    ...
    const char* v_base = (p_base_path != nullptr && p_base_path[0] != '\0') ?
                         p_base_path : G_T20_WEB_DEFAULT_BASE_PATH;
    ...
    String s_down_raw = String(v_base) + "/download/raw";
    String s_down_cfg = String(v_base) + "/download/config";
    String s_down_meta= String(v_base) + "/download/meta";
    String s_down_evt = String(v_base) + "/download/event";
    String s_down_feat= String(v_base) + "/download/feature";
    ...
    v_server->on(s_latest.c_str(), HTTP_GET, [p](AsyncWebServerRequest* request) {
        ...
        if (p->mutex != nullptr && xSemaphoreTake(p->mutex, pdMS_TO_TICKS(50)) == pdTRUE) {
            if (p->latest_vector_valid) {
                ...
                res->print("{\"error\":\"no_latest_feature\"}");
            }
            xSemaphoreGive(p->mutex);
        } else {
            res->print("{\"error\":\"mutex_timeout\"}");
    ...
    v_server->on(s_start.c_str(), HTTP_POST, [p](AsyncWebServerRequest* request) {
        ...
        request->send(ok ? 200 : 500, "application/json; charset=utf-8",
                      ok ? "{\"ok\":true}" : "{\"ok\":false}");
    });
    ...
    auto send_file_if_exists = [](AsyncWebServerRequest* request,
                                  const char* p_path,
                                  const char* p_type) {
        if (p_path == nullptr || p_path[0] == '\0') {
            request->send(404, "application/json; charset=utf-8", "{\"error\":\"empty_path\"}");
            return;
        }
        if (!SD_MMC.exists(p_path)) {
            request->send(404, "application/json; charset=utf-8", "{\"error\":\"not_found\"}");
            return;
        }
        request->send(SD_MMC, p_path, p_type, true);
    };

    v_server->on(s_down_raw.c_str(), HTTP_GET, [p, send_file_if_exists](AsyncWebServerRequest* request) {
        send_file_if_exists(request, p->recorder.raw_path, "application/octet-stream");
    });
    v_server->on(s_down_cfg.c_str(), HTTP_GET, [p, send_file_if_exists](AsyncWebServerRequest* request) {
        send_file_if_exists(request, p->recorder.cfg_path, "application/json; charset=utf-8");
    });
    v_server->on(s_down_meta.c_str(), HTTP_GET, [p, send_file_if_exists](AsyncWebServerRequest* request) {
        send_file_if_exists(request, p->recorder.meta_path, "text/plain; charset=utf-8");
    });
    v_server->on(s_down_evt.c_str(), HTTP_GET, [p, send_file_if_exists](AsyncWebServerRequest* request) {
        send_file_if_exists(request, p->recorder.event_path, "text/plain; charset=utf-8");
    });
    v_server->on(s_down_feat.c_str(), HTTP_GET, [p, send_file_if_exists](AsyncWebServerRequest* request) {
        send_file_if_exists(request, p->recorder.feature_path, "text/csv; charset=utf-8");
    });
...
void T20_webDetach(CL_T20_Mfcc::ST_Impl* p)
{
    ...
    // 현재 단계에서는 route 제거까지는 수행하지 않는다.
    // TODO: 라이브러리별 route 제거 정책 확정 후 실제 detach 구현
