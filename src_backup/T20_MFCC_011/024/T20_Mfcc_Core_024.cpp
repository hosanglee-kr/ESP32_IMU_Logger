...
bool T20_parseConfigJsonText(const char* p_json_text, ST_T20_Config_t* p_cfg_out)
{
    ...
    if (T20_jsonFindBool(p_json_text, "enable_gate", &v_b)) v_cfg.preprocess.noise.enable_gate = v_b;
    if (T20_jsonFindBool(p_json_text, "enable_spectral_subtract", &v_b)) v_cfg.preprocess.noise.enable_spectral_subtract = v_b;
    if (T20_jsonFindBool(p_json_text, "enable", &v_b)) {
        // 경량 파서 한계: 첫 번째 enable이 preemphasis와 충돌 가능
        // 향후 단계 TODO:
        // - 전체 JSON 스키마 정식 파서 전환
        v_cfg.preprocess.preemphasis.enable = v_b;
    }
    if (T20_jsonFindFloat(p_json_text, "alpha", &v_f32)) v_cfg.preprocess.preemphasis.alpha = v_f32;
    if (T20_jsonFindFloat(p_json_text, "gate_threshold_abs", &v_f32)) v_cfg.preprocess.noise.gate_threshold_abs = v_f32;
    if (T20_jsonFindFloat(p_json_text, "spectral_subtract_strength", &v_f32)) v_cfg.preprocess.noise.spectral_subtract_strength = v_f32;
    if (T20_jsonFindInt(p_json_text, "noise_learn_frames", &v_i32)) v_cfg.preprocess.noise.noise_learn_frames = (uint16_t)v_i32;
    if (T20_jsonFindInt(p_json_text, "type", &v_i32)) v_cfg.preprocess.filter.type = (EM_T20_FilterType_t)v_i32;
    if (T20_jsonFindFloat(p_json_text, "cutoff_hz_1", &v_f32)) v_cfg.preprocess.filter.cutoff_hz_1 = v_f32;
    if (T20_jsonFindFloat(p_json_text, "cutoff_hz_2", &v_f32)) v_cfg.preprocess.filter.cutoff_hz_2 = v_f32;
    if (T20_jsonFindFloat(p_json_text, "q_factor", &v_f32)) v_cfg.preprocess.filter.q_factor = v_f32;
    if (T20_jsonFindBool(p_json_text, "filter_enable", &v_b)) v_cfg.preprocess.filter.enable = v_b;
