...
/*
===============================================================================
소스명: T20_Mfcc_Web_020.cpp
버전: v020

[기능 스펙]
- AsyncWeb 기반 제어/다운로드 엔드포인트 제공
- LittleFS 정적 파일(html/css/js) 제공
- polling 기반 live API + websocket/SSE 확장 골격 제공
- 설정 JSON 적용 로직 보강
- recorder rotate 확장 연계를 위한 파일 조회 엔드포인트 유지

[이번 단계 반영]
1. websocket 엔드포인트 골격 추가
2. SSE 엔드포인트 골격 추가
3. 설정 JSON 적용 보강(문자열/숫자/불리언 일부 확장)
4. periodic push hook 추가
5. 향후 고급 push/인증 확장용 주석 보강

[향후 단계 TODO]
- websocket 실제 broadcast 버퍼링 최적화
- SSE reconnect/event-id 처리
- 인증 / 권한 제어
- 전체 JSON 스키마 기반 정식 파서 전환
- 차트용 decimation / aggregation
===============================================================================
*/
...
static void T20_webBuildStatusJson(CL_T20_Mfcc::ST_Impl* p, char* p_buf, size_t p_len)
{
    ...
    snprintf(
        p_buf, p_len,
        "{\"initialized\":%s,\"running\":%s,\"session_state\":%u,\"dropped_frames\":%lu,"
        "\"queued_records\":%lu,\"dropped_records\":%lu,\"written_records\":%lu,"
        "\"web_attached\":%s}",
...
static void T20_webBuildLatestJson(CL_T20_Mfcc::ST_Impl* p, char* p_buf, size_t p_len)
{
    ...
    size_t off = (size_t)n;
    auto append_array = [&](const char* key, const float* arr, uint16_t len, bool with_comma) {
        int m = snprintf(p_buf + off, p_len - off, "\"%s\":[", key);
        if (m < 0 || (size_t)m >= p_len - off) return false;
        off += (size_t)m;
        for (uint16_t i = 0; i < len; ++i) {
            m = snprintf(p_buf + off, p_len - off, (i == 0) ? "%.6f" : ",%.6f", arr[i]);
            if (m < 0 || (size_t)m >= p_len - off) return false;
            off += (size_t)m;
        }
        m = snprintf(p_buf + off, p_len - off, with_comma ? "]," : "]");
        if (m < 0 || (size_t)m >= p_len - off) return false;
        off += (size_t)m;
        return true;
    };
...
bool T20_webPushLiveNow(CL_T20_Mfcc::ST_Impl* p)
{
    ...
    // 현재 단계는 실시간 push 확장 골격만 제공
    // TODO: AsyncEventSource / AsyncWebSocket 실제 broadcast 연결
...
void T20_webPeriodicPush(CL_T20_Mfcc::ST_Impl* p)
{
    ...
    if ((now - p->web_last_push_ms) < G_T20_WEB_PUSH_INTERVAL_MS) {
        return;
    }
...
bool T20_webAttach(CL_T20_Mfcc::ST_Impl* p, void* p_server, const char* p_base_path)
{
    ...
    String s_liveSse    = String(v_base) + "/live/sse";
    String s_liveWs     = String(v_base) + "/live/ws";
    ...
    auto handle_status = [p](AsyncWebServerRequest* request) {
        ...
        if (p->mutex != nullptr && xSemaphoreTake(p->mutex, pdMS_TO_TICKS(50)) == pdTRUE) {
            char buf[G_T20_WEB_JSON_BUFFER_SIZE];
    ...
    v_server->on(s_livePush.c_str(), HTTP_POST, [p](AsyncWebServerRequest* request) {
        bool v_ok = T20_webPushLiveNow(p);
        request->send(v_ok ? 200 : 500, "application/json; charset=utf-8", v_ok ? "{\"ok\":true}" : "{\"ok\":false}");
    });

    v_server->on(s_liveSse.c_str(), HTTP_GET, [](AsyncWebServerRequest* request) {
        request->send(200, "text/plain; charset=utf-8", "SSE skeleton endpoint. TODO: AsyncEventSource integration.");
    });

    v_server->on(s_liveWs.c_str(), HTTP_GET, [](AsyncWebServerRequest* request) {
        request->send(200, "text/plain; charset=utf-8", "WebSocket skeleton endpoint. TODO: AsyncWebSocket integration.");
    });
    ...
    v_server->on(s_config.c_str(), HTTP_POST, [](AsyncWebServerRequest* request){}, nullptr,
        [p](AsyncWebServerRequest* request, uint8_t* data, size_t len, size_t index, size_t total) {
            ...
            if (index + len == total) {
                bool v_ok = T20_applyConfigJsonAdvanced(p, s_body.c_str());
