...
bool CL_T20_Mfcc::start(void)
{
    ...
    if (v1 != pdPASS || v2 != pdPASS) {
        ...
        // queue/mutex는 begin() 성공 상태를 유지할 수 있도록 남길 수도 있지만,
        // 요청대로 stop() 수준 정리 방향이면 cleanup으로 통일하는 편이 더 명확함
        T20_resetRuntimeResources(_impl);
        return false;
    }
...
void CL_T20_Mfcc::printLatest(Stream& p_out) const
{
    ...
    if (!_impl->latest_vector_valid) {
        uint32_t v_dropped = _impl->dropped_frames;
        xSemaphoreGive(_impl->mutex);
        p_out.println(F("No latest feature available."));
        p_out.printf("Dropped Frames: %lu\n", (unsigned long)v_dropped);
        return;
    }
        ...
        while (v_notify_count-- > 0) {
            ...
            if (v_idx >= G_T20_FFT_SIZE) {
                ...
                if (xQueueSend(p->frame_queue, &v_msg, 0) != pdTRUE) {
                    // 큐가 가득 찼으면 가장 오래된 프레임 1개 제거 후 최신 프레임 삽입 시도
