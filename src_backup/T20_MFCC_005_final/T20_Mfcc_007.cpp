...
static void T20_stopTasks(CL_T20_Mfcc::ST_Impl* p);
static void T20_releaseSyncObjects(CL_T20_Mfcc::ST_Impl* p);
static void T20_clearRuntimeState(CL_T20_Mfcc::ST_Impl* p);
static void T20_resetRuntimeResources(CL_T20_Mfcc::ST_Impl* p);
...
void CL_T20_Mfcc::printLatest(Stream& p_out) const
{
    ...
    p_out.print(F("MFCC      : "));
    for (int i = 0; i < G_T20_MFCC_COEFFS; ++i) p_out.printf("%.4f ", v_feat.mfcc[i]);
    p_out.println();

    p_out.print(F("Delta     : "));
    for (int i = 0; i < G_T20_MFCC_COEFFS; ++i) p_out.printf("%.4f ", v_feat.delta[i]);
    p_out.println();

    p_out.print(F("DeltaDelta: "));
    for (int i = 0; i < G_T20_MFCC_COEFFS; ++i) p_out.printf("%.4f ", v_feat.delta2[i]);
    p_out.println();
}
        ...
        while (v_notify_count-- > 0) {
            ...
            if (v_idx >= G_T20_FFT_SIZE) {
                ...
                if (xQueueSend(p->frame_queue, &v_msg, 0) != pdTRUE) {
                    ST_T20_FrameMessage_t v_old_msg;
                    if (xQueueReceive(p->frame_queue, &v_old_msg, 0) == pdTRUE) {
                        p->dropped_frames++; // 가장 오래된 프레임 1개 버림

                        if (xQueueSend(p->frame_queue, &v_msg, 0) != pdTRUE) {
                            p->dropped_frames++; // 최신 프레임도 못 넣음
                        }
                    } else {
                        p->dropped_frames++;
                    }
...
void T20_processTask(void* p_arg)
{
    ...
    for (;;) {
        ...
        float v_mfcc[G_T20_MFCC_COEFFS] = {0};
        float v_delta[G_T20_MFCC_COEFFS] = {0};
        float v_delta2[G_T20_MFCC_COEFFS] = {0};
    ...
    if (p_cfg->preprocess.filter.enable) {
        ...
        if (p_cfg->preprocess.filter.type == EN_T20_FILTER_LPF ||
            ...
            if (p_cfg->preprocess.filter.cutoff_hz_1 <= 0.0f ||
                p_cfg->preprocess.filter.cutoff_hz_1 >= v_nyquist) {
                return false;
            }
        }
        ...
        if (p_cfg->preprocess.filter.type == EN_T20_FILTER_BPF) {
            if (p_cfg->preprocess.filter.cutoff_hz_1 <= 0.0f ||
                ...
                p_cfg->preprocess.filter.cutoff_hz_2 >= v_nyquist) {
                return false;
            }
        }
    }
...
static void T20_buildVector39(const float* p_mfcc, const float* p_delta, const float* p_delta2, float* p_out_vec)
{
    ...
    for (int i = 0; i < G_T20_MFCC_COEFFS; ++i) p_out_vec[v_idx++] = p_mfcc[i];
    for (int i = 0; i < G_T20_MFCC_COEFFS; ++i) p_out_vec[v_idx++] = p_delta[i];
    for (int i = 0; i < G_T20_MFCC_COEFFS; ++i) p_out_vec[v_idx++] = p_delta2[i];
}
